...
from src.consumer.consumer_manager import consumer_manager
...
class AgenticSystemInfrastructure:
    ...
    def start_system(self):
        ...
        try:
            ...
            logger.info("Waiting for initial health check...")
            self._wait_for_initial_health_check()
    ...
    def _show_system_status(self):
        ...
        for agent_uuid, status in consumer_status.items():
            ...
            print(
                f"  - {status['agent_type']}: {running_status} (Messages: {status['message_count']})"
            )
    ...
    def _wait_for_initial_health_check(self, timeout=30):
        """
        ...
        Returns:
            True if health check completed, False if timeout
        """
        start_time = time.time()
        while time.time() - start_time < timeout:
            if health_checker.kafka_status != "unknown":
                logger.info(
                    f"Initial health check completed: Kafka status = {health_checker.kafka_status}"
                )
                return True
            time.sleep(1)

        logger.warning("Timeout waiting for initial health check")
        return False
