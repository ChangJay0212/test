...
class BaseAgent(ABC):
    ...
    def execute_tool(self, tool_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"Agent {self.agent_uuid} attempting to execute tool '{tool_name}' with parameters: {parameters}")
            ...
            try:
                if hasattr(tool, 'validate_parameters'):
                    ...
                    if not is_valid:
                        logger.warning(f"Parameter validation failed for tool '{tool_name}', but proceeding anyway")
                        # Don't fail here, let the tool handle it
                else:
                    logger.info(f"Tool '{tool_name}' doesn't have validate_parameters method")
            except Exception as validation_error:
                logger.warning(f"Parameter validation error for tool '{tool_name}': {validation_error}, proceeding anyway")
            ...
            if result.get("success", False):
                logger.info(f"Tool '{tool_name}' executed successfully by agent {self.agent_uuid}")
            else:
                logger.warning(f"Tool '{tool_name}' execution failed: {result.get('error', 'Unknown error')}")
    ...
    def generate_response(self, prompt: str, use_tools: bool = True, **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            response_data = {
                ...
                "tool_results": {},
                "cost_info": {},
                "model_name": self.llm_engine.model_name if self.llm_engine else "unknown"
            }
            ...
            if use_tools and self.tools:
                ...
                if tool_analysis.get("should_use_tools", False):
                    ...
                    logger.info(f"Agent {self.agent_uuid} decided to use tools: {needed_tools}")
                    
                    # Execute recommended tools
                    for tool_name in needed_tools:
                        if tool_name in self.tool_registry:
                            logger.info(f"Agent {self.agent_uuid} executing tool: {tool_name}")
                            tool_params = self._extract_tool_parameters(prompt, tool_name)
                            ...
                            if tool_result.get("success", False):
                                ...
                                logger.info(f"Agent {self.agent_uuid} successfully used tool: {tool_name}")
                            else:
                                logger.warning(f"Agent {self.agent_uuid} tool failed: {tool_name} - {tool_result.get('error', 'Unknown error')}")
                        else:
                            logger.warning(f"Agent {self.agent_uuid} requested unknown tool: {tool_name}")
                    
                    if response_data["tools_used"]:
                        logger.info(f"Agent {self.agent_uuid} generating response with tool results from: {response_data['tools_used']}")
                        # Generate response with tool results
                        response_data["content"] = self._generate_response_with_tool_results(
                            prompt, response_data["tool_results"], **kwargs
                        )
                    else:
                        logger.info(f"Agent {self.agent_uuid} no tools succeeded, generating normal response")
                        # Generate response without tools
                        response_data["content"] = self.llm_engine.generate_response(prompt, **kwargs)
                else:
    ...
    def _analyze_tool_needs(self, prompt: str) -> Dict[str, Any]:
        ...
        for tool in self.tools:
            if tool.name == "web_search":
                search_keywords = [
                    "search", "find", "lookup", "latest", "current", "recent", "information about",
                    "what is", "tell me about", "查找", "搜尋", "最新", "資訊", "什麼是"
                ]
                if any(keyword in prompt_lower for keyword in search_keywords):
                    recommended_tools.append(tool.name)
                    logger.info(f"Tool {tool.name} recommended based on keywords in: {prompt[:50]}...")
                    
            elif tool.name == "calculator":
                calc_keywords = [
                    "calculate", "compute", "math", "計算", "數學", "plus", "minus", "times", "divided",
                    "+", "-", "*", "/", "=", "×", "÷", "加", "減", "乘", "除"
                ]
                # Also check for numbers that might indicate calculations
                import re
                has_numbers = re.search(r'\d+', prompt)
                has_calc_keywords = any(keyword in prompt_lower for keyword in calc_keywords)
                
                if has_calc_keywords or (has_numbers and any(op in prompt for op in ['+', '-', '*', '/', '×', '÷'])):
                    recommended_tools.append(tool.name)
                    logger.info(f"Tool {tool.name} recommended based on calculation patterns in: {prompt[:50]}...")
                    
            elif tool.name == "weather_check":
                weather_keywords = [
                    "weather", "temperature", "temp", "forecast", "climate", "rain", "sunny", "cloudy",
                    "天氣", "氣溫", "預報", "下雨", "晴天", "陰天", "degrees", "celsius", "fahrenheit",
                    "hot", "cold", "warm", "cool", "humidity", "wind"
                ]
                if any(keyword in prompt_lower for keyword in weather_keywords):
                    recommended_tools.append(tool.name)
                    logger.info(f"Tool {tool.name} recommended based on weather keywords in: {prompt[:50]}...")
    ...
    def _extract_tool_parameters(self, prompt: str, tool_name: str) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Extracting parameters for tool '{tool_name}' from prompt: {prompt[:100]}...")
        ...
        if tool_name == "web_search":
            ...
            filtered_words = [word for word in query_words if word.lower() not in ['what', 'is', 'the', 'tell', 'me', 'about']]
            if filtered_words:
            ...
            for pattern in location_patterns:
                ...
                if match:
                    ...
                    if potential_location.lower() not in ['the', 'weather', 'temperature', 'temp', 'forecast', 'what', 'is', 'check', 'get']:
                        location = potential_location
                        logger.info(f"Found location '{location}' using pattern: {pattern}")
                        break
    ...
    def _generate_response_with_tool_results(self, original_prompt: str, tool_results: Dict[str, Any], **kwargs) -> str:
        """
