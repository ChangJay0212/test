...
class AgentConsumer:
    ...
    def start_consuming(self):
        ...
        logger.info(
            f"Started consumer for {self.agent.agent_type} on topic {self.topic}"
        )
    ...
    def _consume_loop(self):
        ...
        try:
            ...
            self.consumer = kafka_client.get_consumer(
                [self.topic], group_id=f"agent_{self.agent.agent_uuid}"
            )

            logger.info(
                f"Consumer for {self.agent.agent_type} listening on {self.topic}"
            )
            ...
            while self.is_running:
                try:
                    ...
                    if self.is_running:
                        logger.error(
                            f"Error in consumer loop for {self.agent.agent_type}: {e}"
                        )
                    time.sleep(1)
    ...
    def _process_message(self, message: Dict):
        ...
        try:
            ...
            logger.info(
                f"{self.agent.agent_type} processing message #{self.message_count}"
            )
            ...
            logger.info(
                f"{self.agent.agent_type} completed processing message #{self.message_count}"
            )
...
class ConsumerManager:
    ...
    def initialize_agents(self):
        """
        Initialize and register all agent consumers using registry.
        ...
        try:
            ...
            # Create consumers for each registered agent
            for agent_info in registered_agents:
                # Create agent instance using agent factory
                agent = agent_factory.create_agent(agent_info.agent_uuid)
                if agent:
                    consumer = AgentConsumer(agent, agent_info.topic)
                    self.consumers[agent_info.agent_uuid] = consumer
                    logger.info(f"Registered consumer for {agent_info.agent_type}")
                else:
                    logger.error(
                        f"Failed to create agent instance for {agent_info.agent_uuid}"
                    )
            ...
            while self.is_running:
                ...
                for agent_uuid, consumer in self.consumers.items():
                    if not consumer.is_running:
                        logger.warning(
                            f"Consumer {agent_uuid} stopped unexpectedly, restarting..."
                        )
                        consumer.start_consuming()
