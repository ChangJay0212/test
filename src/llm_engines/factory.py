...
from src.llm_engines.gemini_engine import GeminiEngine
from src.llm_engines.ollama_engine import OllamaEngine
...
class LLMEngineFactory:
    ...
    SUPPORTED_ENGINES = {"gemini": GeminiEngine, "ollama": OllamaEngine}
    ...
    def create_for_agent(
        cls, agent_type: str = None, preferred_engine: str = None
    ) -> BaseLLMEngine:
        """
        ...
        if preferred_engine:
            try:
                ...
                logger.warning(
                    f"Failed to create preferred engine {preferred_engine}: {e}"
                )
            ...
            for engine_type in agent_engine_preferences[agent_type]:
                try:
                    ...
                    if validation.get("valid", False):
                        ...
                        logger.warning(
                            f"{engine_type} validation failed: {validation.get('error', 'Unknown error')}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to create {engine_type} for {agent_type}: {e}"
                    )
                    continue
        ...
        for engine_type in cls.SUPPORTED_ENGINES.keys():
            try:
                ...
                validation = cls.validate_engine_config(engine_type)
                logger.info(
                    f"Fallback validation result for {engine_type}: {validation}"
                )
                if validation.get("valid", False):
                    ...
                    logger.warning(
                        f"Fallback {engine_type} validation failed: {validation.get('error', 'Unknown error')}"
                    )
            except Exception as e:
