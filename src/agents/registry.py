...
from src.config.agents import SYSTEM_AGENTS
...
class AgentInfo:
    ...
    description: str
    topic: str
    module_path: str  # 新增：agent 模組路徑
    class_name: str  # 新增：agent 類別名稱
    status: str = "active"
...
class AgentRegistry:
    ...
    def _initialize_default_agents(self):
        """
        Initialize default agents from configuration file.
        """
        for agent_data in SYSTEM_AGENTS:
            # Convert topic name to settings constant if needed
            if agent_data["topic"] == "english_teacher":
                agent_data = agent_data.copy()  # Don't modify original
                agent_data["topic"] = settings.TOPIC_ENGLISH_TEACHER
            elif agent_data["topic"] == "chinese_teacher":
                agent_data = agent_data.copy()
                agent_data["topic"] = settings.TOPIC_CHINESE_TEACHER
    ...
    def register_agent(self, agent_info: AgentInfo) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Agent registered: {agent_info.agent_uuid} -> {agent_info.topic}"
            )
            return True
    ...
    def get_agent_descriptions(self) -> Dict[str, str]:
        ...
        """
        return {agent.agent_type: agent.description for agent in self.agents.values()}
    ...
    def get_all_topics(self) -> List[str]:
        """
        Get all topics managed by registered agents
