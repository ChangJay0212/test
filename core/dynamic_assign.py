...
class DynamicAssigner:
    ...
    def assign_agent(self, message: str) -> Optional[str]:
        ...
        try:
            ...
            assigned_agent_type = self._parse_assignment_response(response, agent_descriptions)
            ...
            if assigned_agent_type:
                ...
                for agent_info in agent_registry.list_agents():
                    if agent_info.agent_type == assigned_agent_type:
                        logger.info(f"Dynamic assignment: '{message[:50]}...' -> {assigned_agent_type}")
    ...
    def _create_assignment_prompt(self, message: str, agent_descriptions: Dict[str, str]) -> str:
        """
        Create prompt for LLM to decide agent assignment
...
Teacher Type:"""
    ...
    def _parse_assignment_response(self, response: str, agent_descriptions: Dict[str, str]) -> Optional[str]:
        """
